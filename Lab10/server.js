...
const server = app.listen(3000, function () {
    ...
    console.log("Lab 10 listening at http://localhost:%s", port);
});
...
io.on("connection", function (socket) {
    socket.on("export", function (exportType) {
        ...
        tweet.find({}, "tweet -_id", function (err, tweets) {
            ...
            if (exportType === "JSON") {
                if (fs.existsSync("ITWS4500-S17-lingeb2-lab9-tweets.json")) {
                    io.emit("fileExists");
                }
                fs.writeFile("ITWS4500-S17-lingeb2-lab9-tweets.json", JSON.stringify(tweets), (err) => {
                    if (err) throw err;
                });
            } else if (exportType === "CSV") {
                if (fs.existsSync("ITWS4500-S17-lingeb2-lab9-tweets.csv")) {
                    io.emit("fileExists");
                }
                var fields = [{
                        label: "created_at",
                ...
                try {
                    ...
                    fs.writeFile("ITWS4500-S17-lingeb2-lab9-tweets.csv", csvData);
                } catch (err) {
                    console.error(err);
                }
            } else {
                if (fs.existsSync("ITWS4500-S17-lingeb2-lab9-tweets.xml")) {
                    io.emit("fileExists");
                }
                fs.writeFile("ITWS4500-S17-lingeb2-lab9-tweets.xml", xml.parse("tweet", tweets));
            }
        });
    });
    socket.on("loadToDatabase", function () {
...
// This route serves our landing page
app.get("/", function (req, res) {
    res.sendFile(__dirname + "/index.html");
});

// This route serves our tweet page
